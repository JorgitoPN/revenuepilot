...
app.post("/webhook/stripe", express.raw({type:"application/json"}), async (req,res)=>{
  try{
    ...
    if(event.type==="checkout.session.async_payment_failed"){
      console.warn(`Async payment failed for checkout session ${event.data.object?.id || "unknown"}`);
    }
...
app.get("/api/opportunities",adminOnly,async(req,res)=>{
  try{res.json(await listOpportunities())}catch(e){res.status(500).json({error:e.message})}
});
...
app.post("/api/autopilot/run",adminOnly,async(req,res)=>{
  try{
    ...
    const result=await runAutopilot({
      getProductBySlug,
      market:String(req.body?.market||process.env.DEFAULT_MARKET||"US").slice(0,30),
      themes,
      autoGenerate:Boolean(req.body?.auto_generate),
      autoPublish:false,
      minScore:Number(req.body?.min_score||80),
      languages
    });
    res.json(result);
  }catch(e){console.error(e);res.status(500).json({error:e.message})}
...
app.get("/api/products",adminOnly,async(req,res)=>{
  try{res.json(await listProducts())}catch(e){res.status(500).json({error:e.message})}
});
app.post("/api/products/generate",adminOnly,async(req,res)=>{
  try{
    ...
    for(let attempt=1;attempt<=2;attempt++){
      ...
      validation=validateProductSpec(spec);
      if(validation.ok) break;
      console.warn(`Product spec QA failed on attempt ${attempt}:`, validation.errors);
    }
    if(!validation?.ok) return res.status(422).json({error:"La especificación generada no superó QA",details:validation?.errors||[]});
    let slug=safeSlug(spec.slug||spec.name);
    if(await getProductBySlug(slug)) slug=`${slug}-${crypto.randomBytes(3).toString("hex")}`;
    const product=await insertProduct({
      ...
      price_cents:Number(spec.price_eur)*100,
...
app.post("/api/products/:id/distribution",adminOnly,async(req,res)=>{
  try{
    ...
    const distribution=await generateDistribution(product,opportunity);
    const updated=await updateDistribution(product.id,distribution);
    res.json(updated);
  }catch(e){console.error(e);res.status(500).json({error:e.message})}
});
...
app.post("/api/products/:id/publish",adminOnly,async(req,res)=>{
  try{
    ...
    const qa=validateProductSpec(p.spec||{});
    if(!qa.ok) return res.status(422).json({error:"El producto no supera QA",details:qa.errors});
    ...
    const updated=await updateProductStatus(p.id,"published");
    res.json(updated);
  }catch(e){console.error(e);res.status(500).json({error:e.message})}
});

app.post("/api/products/:id/checkout",adminOnly,async(req,res)=>{
  try{
    if(!stripe) return res.status(503).json({error:"STRIPE_SECRET_KEY no configurado"});
    const p=await getProductById(req.params.id);
    if(!p) return res.status(404).json({error:"Producto no encontrado"});
    const suffix=crypto.randomBytes(6).toString("hex").slice(0,8);
    const session=await stripe.checkout.sessions.create({
      mode:"payment",
      line_items:[{price_data:{currency:"eur",unit_amount:p.price_cents,product_data:{name:p.name,description:p.tagline}},quantity:1}],
      success_url:`${BASE_URL}/delivery.html?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url:`${BASE_URL}/`,
      customer_creation:"if_required",
      metadata:{product_id:p.id,product_slug:p.slug},
      integration_identifier:`revenuepilot_${suffix}`
    });
...
app.get("/shop/:slug",async(req,res)=>{
  try{
    ...
    if(!p || p.status!=="published") return res.status(404).send("Producto no disponible");
    res.type("html").send(renderShopLanding(p));
  }catch(e){res.status(500).send("Error al cargar la oferta")}
});
...
app.get("/buy/:slug",async(req,res)=>{
  try{
    ...
    const session=await stripe.checkout.sessions.create({
      ...
      line_items:[{price_data:{currency:"eur",unit_amount:p.price_cents,product_data:{name:p.name,description:p.tagline}},quantity:1}],
      success_url:`${BASE_URL}/delivery.html?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url:`${BASE_URL}/shop/${encodeURIComponent(p.slug)}`,
      customer_creation:"if_required",
      metadata:{product_id:p.id,product_slug:p.slug},
      integration_identifier:`revenuepilot_${suffix}`
    });
    res.redirect(303,session.url);
  }catch(e){console.error(e);res.status(500).send("No se pudo iniciar el checkout")}
});
...
app.post("/api/delivery/confirm",async(req,res)=>{
  try{
    ...
    if(!id.startsWith("cs_")) return res.status(400).json({error:"Sesión inválida"});
    const session=await stripe.checkout.sessions.retrieve(id);
    if(session.payment_status!=="paid") return res.status(402).json({error:"El pago aún no figura como pagado"});
    const productId=session.metadata?.product_id;
    const p=productId?await getProductById(productId):null;
    if(!p) return res.status(404).json({error:"Producto no encontrado"});
    await insertOrder({
      stripe_session_id:session.id,product_id:p.id,
      customer_email:session.customer_details?.email||session.customer_email||null,
      amount_total:session.amount_total||p.price_cents,currency:session.currency||"eur",payment_status:session.payment_status
    });
    const token=signDownload({product_id:p.id,session_id:session.id,exp:Date.now()+72*3600_000});
    res.json({product_name:p.name,download_url:`/download/${encodeURIComponent(token)}`});
  }catch(e){console.error(e);res.status(400).json({error:"No se pudo verificar el pago"})}
});

app.get("/download/:token",async(req,res)=>{
  try{
    const payload=verifyDownload(req.params.token);
    const p=await getProductById(payload.product_id);
    ...
    if(stripe){
      ...
      if(session.payment_status!=="paid" || session.metadata?.product_id!==p.id) return res.status(403).send("Compra no verificada");
    }
    res.setHeader("Content-Disposition",`attachment; filename="${p.slug}.html"`);
    res.type("html").send(renderStandaloneProduct(p));
  }catch(e){res.status(403).send(e.message)}
});

app.get("/api/orders",adminOnly,async(req,res)=>{
  try{res.json(await listOrders())}catch(e){res.status(500).json({error:e.message})}
});

app.get("/p/:slug",adminOnly,async(req,res)=>{
  try{
    const p=await getProductBySlug(req.params.slug);
    if(!p) return res.status(404).send("Producto no encontrado");
    res.type("html").send(renderStandaloneProduct(p));
  }catch(e){res.status(500).send("Error al cargar producto")}
});
app.get("/api/products/:id/export",adminOnly,async(req,res)=>{
  try{
    const p=await getProductById(req.params.id);
    if(!p) return res.status(404).json({error:"Producto no encontrado"});
    res.setHeader("Content-Disposition",`attachment; filename="${p.slug}.html"`);
